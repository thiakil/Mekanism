		...
		for (; ; ) {
			...
			if (key.equalsIgnoreCase("newmtl")) {
				...
				if (key.equalsIgnoreCase("Kd") || !hasSetColor) {
					...
					Mekanism.logger.info("OBJModel: A color has already been defined for material '{}' in '{}'. The color defined by key '{}' will not be applied!", material.getName(), new ResourceLocation(domain, path).toString(), key);
				}
				...
				if (key.equalsIgnoreCase("map_Kd") || !hasSetTexture) {
					...
					Mekanism.logger.info("OBJModel: A texture has already been defined for material '{}' in '{}'. The texture defined by key '{}' will not be applied!", material.getName(), new ResourceLocation(domain, path).toString(), key);
				}
				...
				if (!unknownMaterialCommands.contains(key)) {
					...
					Mekanism.logger.info("OBJLoader.MaterialLibrary: key '{}' (model: '{}') is not currently supported, skipping", key, new ResourceLocation(domain, path));
				}
			}
		}
	}
