...
public class Parser {
	...
	public GlowBJModel parse() throws IOException {
		String currentLine = "";
			...
			try {
				...
				if (key.equalsIgnoreCase("mtllib")) {
					...
					if (this.materialLibrary.materials.containsKey(data)) {
						...
						Mekanism.logger.error("OBJModel.Parser: (Model: '{}', Line: {}) material '{}' referenced but was not found", objFrom, lineNum, data);
					}
					usemtlCounter++;
				...
				} else if (key.equalsIgnoreCase("vt")) // Vertex Textures: u [v] [w] - v/w Defaults to 0
				{
					...
					if (texCoord.u < 0.0f || texCoord.u > 1.0f || texCoord.v < 0.0f || texCoord.v > 1.0f)
						throw new GlowBJModel.UVsOutOfBoundsException(this.objFrom);
					this.texCoords.add(texCoord);
				} else if (key.equalsIgnoreCase("f")) // Face Elements: f v1[/vt1][/vn1] ...
				{
					if (splitData.length > 4)
						Mekanism.logger.warn("OBJModel.Parser: found a face ('f') with more than 4 vertices, only the first 4 of these vertices will be rendered!");
					...
					face.glowMode = glowMode;
					face.lightLevel = lightLevel;
					...
					if (data.equalsIgnoreCase("off"))
					{
						...
						lightLevel = Integer.parseInt(data);
						glowMode = lightLevel > 0 && lightLevel <= 15;
					}
				} else {
					if (!unknownObjectCommands.contains(key)) {
						unknownObjectCommands.add(key);
						Mekanism.logger.info("OBJLoader.Parser: command '{}' (model: '{}') is not currently supported, skipping. Line: {} '{}'", key, objFrom, lineNum, currentLine);
					}
				}
		...
		return new GlowBJModel(this.materialLibrary, this.objFrom);
	}
}
