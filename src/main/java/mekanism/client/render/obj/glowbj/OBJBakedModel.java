...
public class OBJBakedModel implements IBakedModel {
	private final GlowBJModel model;
	...
	public OBJBakedModel(GlowBJModel model, IModelState state, VertexFormat format, ImmutableMap<String, TextureAtlasSprite> textures) {
		this.model = model;
		this.state = state;
...
//                FMLLog.info("Group: %s u: [%f, %f] v: [%f, %f]", g.name, g.minUVBounds[0], g.maxUVBounds[0], g.minUVBounds[1], g.maxUVBounds[1]);
		...
		for (Face f : faces) {
			...
			UnpackedBakedQuad.Builder builder = new UnpackedBakedQuad.Builder(f.glowMode ? DefaultVertexFormats.POSITION_TEX_LMAP_COLOR : format);
			builder.setContractUVs(true);
			builder.setQuadOrientation(EnumFacing.getFacingFromVector(f.getNormal().x, f.getNormal().y, f.getNormal().z));
			builder.setTexture(sprite);
			Normal faceNormal = f.getNormal();
			putVertexData(builder, f.verts[0], faceNormal, TextureCoordinate.getDefaultUVs()[0], sprite, f.glowMode ? f.lightLevel : 0);
			putVertexData(builder, f.verts[1], faceNormal, TextureCoordinate.getDefaultUVs()[1], sprite, f.glowMode ? f.lightLevel : 0);
			putVertexData(builder, f.verts[2], faceNormal, TextureCoordinate.getDefaultUVs()[2], sprite, f.glowMode ? f.lightLevel : 0);
			putVertexData(builder, f.verts[3], faceNormal, TextureCoordinate.getDefaultUVs()[3], sprite, f.glowMode ? f.lightLevel : 0);
			if (f.glowMode)
			{
	...
	private final void putVertexData(UnpackedBakedQuad.Builder builder, Vertex v, Normal faceNormal, TextureCoordinate defUV, TextureAtlasSprite sprite, int lightLevel) {
		for (int e = 0; e < format.getElementCount(); e++) {
			switch (format.getElement(e).getUsage()) {
				...
				case COLOR:
					...
					break;
				case UV:
					if (format.getElement(e).getIndex() == 0) { //UV
						...
						if (lightLevel > 0){
							...
							float skyLight = ((float) ((lightLevel >> 20) & 15) * 32) / 65535; //Found this in Forge
