...
public class AE2ConfigCardCompatCableBus extends AE2ConfigCardCompatBlock {
    ...
    private static final Item INTERFACE_PART = BuiltInRegistries.ITEM.get(AEPartIds.INTERFACE);
    private static final Item INTERFACE_BLOCK = BuiltInRegistries.ITEM.get(AEBlockIds.INTERFACE);
    private static final Item PATTERN_PART = BuiltInRegistries.ITEM.get(AEPartIds.PATTERN_PROVIDER);
    private static final Item PATTERN_BLOCK = BuiltInRegistries.ITEM.get(AEBlockIds.PATTERN_PROVIDER);
    ...
    public String getConfigCardName() {
        ...
        if (INTERFACE_PART == partItem) {
            partItem = INTERFACE_BLOCK;
        } else if (PATTERN_PART == partItem) {
            partItem = PATTERN_BLOCK;
        }
